.PHONY: build release release-tag release-wait release-tap test clean
...
release: release-tag release-wait release-tap
...
release-tap:
	...
	@SHA_ARM=$$(gh release view v$(VERSION) -R $(REPO) --json assets -q '.assets[] | select(.name | contains("aarch64-apple-darwin")) | .digest' | sed 's/sha256://') && \
		...
		sed -i '' 's/version ".*"/version "$(VERSION)"/' $(HOMEBREW_TAP)/Formula/garmin.rb && \
		sed -i '' 's|garmin-v[0-9.]*-aarch64|garmin-v$(VERSION)-aarch64|g' $(HOMEBREW_TAP)/Formula/garmin.rb && \
		sed -i '' 's|garmin-v[0-9.]*-x86_64|garmin-v$(VERSION)-x86_64|g' $(HOMEBREW_TAP)/Formula/garmin.rb && \
		sed -i '' 's|download/v[^/]*/garmin|download/v$(VERSION)/garmin|g' $(HOMEBREW_TAP)/Formula/garmin.rb && \
		awk -v arm="$$SHA_ARM" -v x86="$$SHA_X86_MAC" -v linux="$$SHA_LINUX" \
			...
			$(HOMEBREW_TAP)/Formula/garmin.rb > $(HOMEBREW_TAP)/Formula/garmin.rb.tmp && \
		mv $(HOMEBREW_TAP)/Formula/garmin.rb.tmp $(HOMEBREW_TAP)/Formula/garmin.rb
	@cd $(HOMEBREW_TAP) && git add -A && git commit -m "garmin $(VERSION)" && git pull --rebase && git push
	@echo ""
